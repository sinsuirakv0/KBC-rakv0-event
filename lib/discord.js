...
export async function notifyEventBot({
  ...
  const endpoints = [
    { url: BOT_EVENT_UPDATE_URL, secret: BOT_EVENT_UPDATE_SECRET, name: 'Discord bot' },
    { url: LINE_BOT_EVENT_UPDATE_URL, secret: LINE_BOT_EVENT_UPDATE_SECRET, name: 'LINE bot' },
  ...
  await Promise.all(endpoints.map(async endpoint => {
    try {
      ...
      if (!res.ok) {
        console.warn(`${endpoint.name} event notification failed: HTTP ${res.status}`);
      }
    } catch (e) {
